...
var wordsList = function wordsList(str, subStr) {
    var reg = new RegExp('\\.|,|\\?|!|:|;|"', 'gui');
    var arr = str.replace(reg, '').toLowerCase().split(' ').filter(function (arrItem) {
      return arrItem.indexOf(subStr) > -1;
    });
    var res = new Set();
...
// console.log(wordsList(myLongStr));
...
var getLocalDay = function getLocalDay(d) {
    var date = new Date(d);
    var day = date.getDay();
    if (day === 0) day = 7;
    return day;
...
var getDateAgo = function getDateAgo(d, days) {
    var date = new Date(d);
    date.setDate(date.getDate() - days);
    return date.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, '$1');
};
...
let car = new Car(2000, 'GT86', 'Toyota', 2012);
let car2 = new Car(2200, 'Freelander 2', 'Land Rover', 2020);
...
var testPerformance = function testPerformance(iterations, func) {
    var time = Date.now();

    if (typeof func === 'function') for (var i = iterations; i--;) {
      func();
    }
